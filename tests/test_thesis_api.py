...
def test_create_thesis(client):
    response = client.post('/v1/api/thesis/update', json={
        'title': 'Test Thesis',
        'author': 'John Doe',
...
def test_get_all_theses(client):
    ...
    client.post('/v1/api/thesis/update', json={
        'title': 'Test Thesis',
        'author': 'John Doe',
        'abstract': 'An example thesis for testing.',
        'status': 'In Progress',
...
def test_get_single_thesis(client):
    # Create a sample thesis first
    post_response = client.post('/v1/api/thesis/update', json={
        'title': 'Test Thesis',
        'author': 'John Doe',
        'abstract': 'An example thesis for testing.',
        'status': 'In Progress',
    ...
    assert post_response.status_code == 201, f"Unexpected status code: {post_response.status_code}, Response: {post_response.data.decode()}"
    post_data = post_response.get_json()
    assert post_data is not None, f"Response JSON is None, Response: {post_response.data.decode()}"
...
def test_update_thesis(client):
    ...
    # Update the thesis
    response = client.put(f'/v1/api/thesis/{thesis_id}', json={
...
def test_delete_thesis(client):
    # Create a sample thesis first
    post_response = client.post('/v1/api/thesis', json={
        'title': 'Test Thesis',
        'author': 'John Doe',
        'abstract': 'An example thesis for testing.',
        'status': 'In Progress',
