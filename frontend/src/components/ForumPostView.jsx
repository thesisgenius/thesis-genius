...
const ForumPostView = () => {
  ...
  const addComment = async () => {
    try {
      ...
      setPost((prev) => ({
        ...
        comments: {
          ...prev.comments,
          results: [...(prev.comments?.results || []), newComment],
        },
      }));
      setCommentContent("");
    } catch (error) {
  ...
  if (!post || !post.user) {
    console.warn(
      "Post or post.user is missing, ownership cannot be determined.",
    );
    ...
    <div className="post-view">
      ...
      {isOwner && (
        <div>
          <button
            style={styles.button}
            onClick={() => navigate(`/forum/posts/${post.post.id}/edit`)}
          >
            Edit Post
          </button>
        </div>
        ...
        {post.comments?.results?.length > 0 ? (
          post.comments.results.map((comment) => (
            <div key={comment.id || Math.random()} className="comment">
              <p>{comment.content}</p>
              <small>
                Comment by User {comment.username || "Unknown User"}
              </small>
