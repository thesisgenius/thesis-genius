...
def create_thesis():
    ...
    try:
        submission_date = datetime.strptime(submission_date, '%Y-%m-%d').date() if submission_date else None
        thesis = Thesis(title=title, author=author, abstract=abstract, status=status, submission_date=submission_date)
        db.session.add(thesis)
        db.session.commit()
        return jsonify({'id': thesis.id, 'message': 'Thesis created successfully.'}), 201
...
def get_all_theses():
    ...
    return jsonify([{
        ...
        'submission_date': thesis.submission_date.strftime('%Y-%m-%d') if thesis.submission_date else None
...
def update_thesis(id):
    ...
    try:
        ...
        if 'submission_date' in data:
            thesis.submission_date = datetime.strptime(data['submission_date'], '%Y-%m-%d').date() if data['submission_date'] else None
        db.session.commit()
